...
public class MbrCmpEquitySelection extends JFrame {
	...
	EquityAssetType[] aryEquityAssetType;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MbrCmpEquitySelection frame = new MbrCmpEquitySelection();
	...
	public MbrCmpEquitySelection() {}

	public MbrCmpEquitySelection(DatabaseVariables dbEqVar) {
		...
		btnEnter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (dbEqVar.getGoodToGo()) {
					...
					case "D":
						sqlQuery = "Select * From dbo.EquityDataDE";
						sqlQuery2 = "Select * From dbo.EquityTotalDE";
						title = "DE_Equity";
						break;
					case "P":
						sqlQuery = "Select * From dbo.EquityDataPA";
						sqlQuery2 = "Select * From dbo.EquityTotalPA";
						title = "PA_Equity";
						break;
					case "W":
						sqlQuery = "Select * From dbo.EquityDataWV";
						sqlQuery2 = "Select * From dbo.EquityTotalWV";
						title = "WV_Equity";
						break;
					}
					//POI - Generate Excel Equity Spreadsheet...
	...
	public void VerifyEquityAsset(DatabaseVariables dbEqVar) {
		if (!txtAorE.getText().equals("E")) {
			JOptionPane.showMessageDialog(null, "Enter E=Equity");
			dbEqVar.setGoodToGo(false);
		}
	...
	public void GetAmountTotals(DatabaseVariables dbEqVar) {
		...
		GetNetTotals netTotals = new GetNetTotals(dbEqVar);
		netTotals.getClass();
	...
	public void LoadTable(DatabaseVariables dbEqVar) {
		// Define and populate SQL database table array in place of array for SQL...
		aryEquityAssetType = new EquityAssetType[10];
		for (int i = 0; i < aryEquityAssetType.length; i++) { // Equity loop...
		...
		if (dbEqVar.getGoodToGo()) {
			...
			try {
				...
				SQLServerPreparedStatement preStmt = (SQLServerPreparedStatement) connection
						.prepareStatement("Exec Member_Company_Equity_Data ?,?");
			...
			// Run Stored Procedure SQL...
			try {
				// Connect to database...
				connection = sqlConnection.dbConnector();

				SQLServerPreparedStatement preStmt = (SQLServerPreparedStatement) connection
						.prepareStatement("Exec Member_Company_Equity_Totals ?");
				preStmt.setString(1, dbEqVar.getDbStateCode());
				preStmt.execute();
				dbEqVar.setGoodToGo(true);
			} catch (SQLException e) {
